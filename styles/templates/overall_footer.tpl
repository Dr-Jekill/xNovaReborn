<div id="siteFooter">
    <div class="content">
        <div class="fleft textLeft">
            <a href="?page=changelog" onclick="mrbox('./?page=changelog',800);" class="tips tooltip" style="position:relative; color:#00e5fc;" data-role="PGtooltip" data-direction="top" data-content="<span class=&quot;mrtooltip&quot; style=&quot;top:-20px;&quot;>Notas de la version</span>">{$VERSION}</a>
            <a class="homeLink" href="http://parallelgames.cubava.cu/" target="">© 2020 ParallelGames. Todos los derechos reservados.</a>
        </div>
        <div class="fright textRight">
            {if !empty($lm_credits)}<a href="?page=creditos">{$lm_credits}</a>|{/if}
            <a href="?page=faq">FAQ</a>|
            <a href="{$forum_url}" target="_blank">{$lm_forums}</a>|
            <a href="./?page=rules" target="_blank">Reglas</a>
            {if !CheckModule(21)}|
            <a href="?page=banned" style="color:#ff3c3c">Baneados</a>
            {/if}
        </div>
        <br class="clearfloat">
    </div>
...
<script type="text/javascript" src="{$cd}scripts/JisGreat.js?v={$REV}"></script>
