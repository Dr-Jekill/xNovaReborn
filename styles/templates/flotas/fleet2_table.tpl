...
<div id="planeta_small" style="background-image: url(styles/theme/{$Raza_skin}/adds/flotas3.jpg);">
...
<td style="width:50%;margin:0;padding:0;">
        		<table width="270px"><tr>
        			{foreach item=MissionName key=MissionID from=$MissionSelector}
                     ...
                     <img data-role="PGtooltip" data-direction="top" data-content="{$MissionName}" src="styles/theme/{$Raza_skin}/imagenes/infos/{$MissionID}.png" /><br /><input id="radio_{$MissionID}" type="radio" name="mission" value="{$MissionID}" {if $mission == $MissionID}checked="checked"{/if} style="width:60px;">
                  </label>
						</td>
					{/foreach}	
						<tr>
