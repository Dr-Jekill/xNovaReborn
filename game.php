...
require_once(ROOT_PATH.'includes/functions/SortUserMoons.php');
...
// ----------------------------------------------------------------------------------------------------------------------------------------------//
	...
	case "movement":
		...
		include_once(ROOT_PATH . 'includes/pages/class.ShowFleetPages.php');
		ShowFleetPages::ShowMovementPage();
	break;
// ----------------------------------------------------------------------------------------------------------------------------------------------//
	case 'buildings':
...
// ----------------------------------------------------------------------------------------------------------------------------------------------//
	...
	$mode = request_var('load', '');
		...
		default:
			...
			include_once(ROOT_PATH . 'includes/pages/class.ShowResourcesPage.php');
			new ShowResourcesPage();
			break;
		}
	break;
...
// ----------------------------------------------------------------------------------------------------------------------------------------------//
	...
	default:
		...
		if(file_exists(ROOT_PATH . 'includes/modules/xrb_' . $page . '/pg_' . $page . '.php')){
			if(!isModuleActive($page))
				message($LNG['sys_module_inactive'],"?page=overview", 3, true, true);
				
			include_once(ROOT_PATH . 'includes/modules/xrb_' . $page . '/pg_' . $page . '.php');
			$pgmod = new XRBModule();
		}else{
