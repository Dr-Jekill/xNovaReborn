...
if(!defined('INSIDE')) die('Hacking attempt!');
	...
	function GetBuildingTime ($USER, $PLANET, $Element, $Destroy = false)
	{
		...
		if(in_array($Element, $reslist['build'])){
			...
			if($USER['raza']==1){#voltra -10% tiempo contruccion naves
				$procent = $time * 0.1;
				$time -= $procent;
			}
			...
			$time			= $Cost / ($CONF['game_speed'] * (1 + $PLANET[$resource[31]])) + (0.3 - $comandante_tecnos);

			if($PLANET[$resource[6]] > 0){
				$procent = $time * (0.08 * $PLANET[$resource[6]]);
				$time -= $procent;
			}
			...
			if($USER['raza']==1){#voltra -10% tiempo investigacion
				$procent = $time * 0.1;
				$time -= $procent;
			}		
        }
