...
class Session
{
	...
	function GetTempSessionFromDB(){
		global $db;

		return $db->uniquequery("SELECT * FROM ".SESSION." WHERE `sess_id` = '".session_id()."' AND `user_id` = '".$_SESSION['id']."' AND user_side LIKE '%lobby.php%';");
	}
	...
	function CreateTempSession($ID, $Username, $email){
		global $db;
		$Path					= $this->GetPath();
	...
	function UpdateTempSession(){
		...
		$SQL .= "`user_side` = '".$db->sql_escape($_SESSION['path'])."' ";
		$SQL .= "WHERE ";
		$SQL .= "`sess_id` = '".session_id()."';";
		$db->query($SQL);
				
		$_SESSION['last']	= $this->GetTempSessionFromDB(); #Hacer para TempSession
		if(empty($_SESSION['last']) || !$this->CompareIPs($_SESSION['last']['user_ip'])) {
			$this->DestroySession();
			$this->ErrorMessage(2);
		}
		...
		$SQL  = "UPDATE ".SESSION."  SET ";
		$SQL .= "`user_ip` = '".$_SERVER['REMOTE_ADDR']."', ";
		$SQL .= "`user_side` = '".$db->sql_escape($_SESSION['path'])."', ";
		/*$SQL .= "s.`user_ua` = '".$db->sql_escape($_SERVER['HTTP_USER_AGENT'])."', ";
		$SQL .= "s.`user_method` = '".$_SERVER["REQUEST_METHOD"]."', ";*/
		$SQL .= "`user_lastactivity` = '".TIMESTAMP."' ";
		$SQL .= "WHERE ";
		$SQL .= "`sess_id` = '".session_id()."';";
		$db->query($SQL);
