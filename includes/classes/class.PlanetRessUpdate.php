	...
	private function UpdateRessource()
	{
		...
		else
		{
			...
			foreach($reslist['prod'] as $id => $ProdID)
			{
				...
				if($this->USER['raza']==0){
					$Caps['metal_perhour'] 		-= ($Caps['metal_perhour'] * 0.1);
					$Caps['crystal_perhour'] 	-= ($Caps['crystal_perhour'] * 0.1);
					$Caps['norio_perhour'] 		-= ($Caps['norio_perhour'] * 0.1);
					$Caps['deuterium_perhour'] 	-= ($Caps['deuterium_perhour'] * 0.1);
				}
			}
			...
			if($USER['geologe'] >= 1){
				$this->PLANET['metal_perhour'] += ($this->PLANET['metal_perhour'] * 0.1);
				$this->PLANET['crystal_perhour'] += ($this->PLANET['crystal_perhour'] * 0.1);
				$this->PLANET['deuterium_perhour'] += ($this->PLANET['deuterium_perhour'] * 0.1);
				$this->PLANET['norio_perhour'] += ($this->PLANET['norio_perhour'] * 0.1);
			}

			$this->PLANET['energy_used']	= $Caps['energy_used'];	
			
			if($this->USER['engineer'] >= 1) {
				$this->PLANET['energy_used']	-= ($Caps['energy_used'] * 0.1);	
			}

			if($this->USER[$resource[122]] >= 1){
				$this->PLANET['metal_perhour'] += $this->PLANET['metal_perhour'] * (0.01 * $this->USER[$resource[122]]);
				$this->PLANET['crystal_perhour'] += $this->PLANET['crystal_perhour'] * (0.0066 * $this->USER[$resource[122]]);
				$this->PLANET['norio_perhour'] += $this->PLANET['norio_perhour'] * (0.0033 * $this->USER[$resource[122]]);
				$this->PLANET['deuterium_perhour'] += $this->PLANET['deuterium_perhour'] * (0.0016 * $this->USER[$resource[122]]);
			}
...
if ( $this->USER['technocratic_time'] <= $hoytime AND $this->USER['technocratic'] == 1) {
	...
	public function SavePlanetToDB($USER = NULL, $PLANET = NULL)
	{
		...
		$Qry	= "UPDATE ".PLANETS." as p, ".USERS." as u, ".SESSION." as s SET
