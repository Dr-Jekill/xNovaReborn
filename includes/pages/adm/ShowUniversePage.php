...
function ShowUniversePage() {
	...
	if($_REQUEST['action'] == 'delete' && !empty($_REQUEST['id']) && $_REQUEST['id'] != ROOT_UNI) {
		...
		$ID	= (int) $_REQUEST['id'];
		$db->query("INSERT INTO ".CONFIG." (`uni`, `VERSION`, `uni_name`, `game_name`, `close_reason`, `trade_charge`, `server_created`) VALUES
		(NULL, '".$CONF['VERSION']."', 'Uni', '".$CONF['game_name']."', '', '0.3', '".TIMESTAMP."');");
		...
		$Backup	= serialize(array(
			...
			'CONFIG'		=> $db->fetchquery("SELECT * FROM ".CONFIG." WHERE `uni` = ".$_REQUEST['id'].";", array('close_reason')),
