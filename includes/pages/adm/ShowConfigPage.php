...
{
	global $CONF, $LNG, $USER, $LANG, $db, $reslist;
	if (!empty($_POST))
	{
		...
		$CONF['game_sound'] 			= isset($_POST['sound']) && $_POST['sound'] == 'on' ? 1 : 0;
		$CONF['game_sound'] 			= isset($_POST['sound']) && $_POST['sound'] == 'on' ? 1 : 0;
		$CONF['resources_images'] 			= isset($_POST['resources_images']) && $_POST['resources_images'] == 'on' ? 1 : 0;
		$CONF['build_images'] 			= isset($_POST['build_images']) && $_POST['build_images'] == 'on' ? 1 : 0;
		...
		$CONF['trade_charge']			= request_var('trade_charge', 0) / 100;
		$CONF['min_point_for_alliance']	= request_var('points_alliance', 0);
		$CONF['queue_build_init']		= request_var('queue_build_init', 0);
		$CONF['queue_build_commander']	= request_var('queue_build_commander', 0);
		$CONF['queue_research_init']	= request_var('queue_research_init', 0.0);
		$CONF['queue_research_commander']= request_var('queue_research_commander', 0.0);
		...
		update_config(array(
			...
			'min_point_for_alliance'=> $CONF['min_point_for_alliance'],
			'queue_build_init'		=> $CONF['queue_build_init'],
			'queue_build_commander'	=> $CONF['queue_build_commander'],
			'queue_research_init'	=> $CONF['queue_research_init'],
			'queue_research_commander'=> $CONF['queue_research_commander'],
			'game_sound'			=> $CONF['game_sound'],
			'resources_images'		=> $CONF['resources_images'],
			'build_images'			=> $CONF['build_images'],
	...
	foreach($reslist['fleet'] as $Element) {
		...
		$Ships[]        = array(
			...
			'datas' => json_encode(array('id' => $Element, 'name' => $LNG['tech'][$Element], 'descriptions' => $LNG['res']['descriptions'][$Element])),
		);
	...
	$template->assign_vars(array(
		...
		'se_min_points_for_alliance'	=> $LNG['se_min_points_for_alliance'],
		'se_reg_closed'					=> $LNG['se_reg_closed'],
		'se_verfiy_mail'				=> $LNG['se_verfiy_mail'],
		'se_min_build_time_info'		=> $LNG['se_min_build_time_info'],
		'se_min_points_for_alliance_info'=> $LNG['se_min_points_for_alliance_info'],
		...
		'se_build_parameters'			=> $LNG['se_build_parameters'],
		'se_research_parameters'			=> $LNG['se_research_parameters'],
		...
		'se_build_default_queue'		=> $LNG['se_build_default_queue'],
		'se_build_commander_queue'		=> $LNG['se_build_commander_queue'],
		'se_research_default_queue'		=> $LNG['se_research_default_queue'],
		'se_research_commander_queue'	=> $LNG['se_research_commander_queue'],
		'se_game_sound_info'			=> $LNG['se_game_sound_info'],
		'se_game_sound'					=> $LNG['se_game_sound'],
		'game_name'						=> $CONF['game_name'],
		'uni_name'						=> $CONF['uni_name'],
		...
		'bgm_file'           			=> $CONF['bgm_file'],
		'trade_allowed_ships'        	=> $CONF['trade_allowed_ships'],
		'trade_charge'		        	=> $CONF['trade_charge'] * 100,
		'points_alliance'		        => $CONF['min_point_for_alliance'],
		'queue_build_init'		        => $CONF['queue_build_init'],
		'queue_build_commander'		    => $CONF['queue_build_commander'],
		'queue_research_init'		    => $CONF['queue_research_init'],
		'queue_research_commander'		=> $CONF['queue_research_commander'],
		...
		'se_game_sound'					=> $LNG['se_game_sound'],
		'resources_images'				=> $CONF['resources_images'],
		'resources_images_text'			=> 'Activar imagenes de recursos?',
		'resources_images_info'			=> 'Este es un contenido descargable de la pagina oficial de ParallelGames. Son las imagenes del banner de los recursos del planeta.',
		'build_images'					=> $CONF['build_images'],
		'build_images_text'				=> 'Activar imagenes de construcciones?',
		'build_images_info'				=> 'Este es un contenido descargable de la pagina oficial de ParallelGames. Son las imagenes del banner de las construcciones del planeta.',
