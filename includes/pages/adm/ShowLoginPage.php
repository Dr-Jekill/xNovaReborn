...
function ShowLoginPage()
{
	...
	if($_POST){
		...
		{
		        $login = $db->uniquequery("SELECT `id`, `username`, `dpath`, `authlevel`, `id_planet` FROM ".USERS." WHERE `username` = '".$_SESSION['username']."' AND `password` = '".md5($_REQUEST['admin_pw'])."';");

		        if($login['authlevel'] == AUTH_USR)redirectTo('game.php');
