...
class ShowResearchPage
{
	...
	public function CancelBuildingFromQueue($PlanetRess){
		...
		if($PLANET['id'] == $CurrentQueue[0][4])
		{
			...
			$db->query("UPDATE ".PLANETS." SET `metal` = `metal` + '".$costs['metal']."', `crystal` = `crystal` + '".$costs['crystal']."', `deuterium` = `deuterium` + '".$costs['deuterium']."',`norio` = `norio` + '".$costs['norio']."' WHERE `id` = '".$CurrentQueue[0][4]."';");
		}
	...
	public function RemoveBuildingFromQueue($QueueID, $PlanetRess)
	{
		...
		$Element       	= $CurrentQueue[$QueueID - 2][0];
      	...
      	$Planet_ID   	= $CurrentQueue[$QueueID - 1][4];
	    ...
	    if($PLANET['id'] == $Planet_ID){
	        ...
	        $db->query("UPDATE ".PLANETS." SET `metal` = `metal` + '".$Resses['metal']."', `crystal` = `crystal` + '".$Resses['crystal']."', `deuterium` = `deuterium` + '".$Resses['deuterium']."',`norio` = `norio` + '".$Resses['norio']."' WHERE `id` = '".$Planet_ID."';");
	    }
	...
	public function AddBuildingToQueue($Element, $AddMode = true){
		global $PLANET, $USER, $resource,$db, $CONF;
		...
		if($ActualCount >= $CONF['queue_research_init'] and $USER['commander'] <= 0) {
			die(header("location:game.php?page=buildings&mode=research"));  
		}
				
		if($CONF['queue_research_commander'] <= $ActualCount)
			return false;
		...
		if (is_array($CurrentQueue)) {	
			foreach($CurrentQueue as $BuildArray) {
				...
				$ScriptData[] = array(
					...
					'planet' => $PlanetName, 'endtime' => $BuildArray[3], 
					'reload' => in_array($BuildArray[0], array(123)),
					'mode' => $LNG['bd_upgradin'],
				);
			}
			return $ScriptData;
		}
	...
	public function __construct()
	{
		...
		if(!empty($Element) && $bContinue && $USER['urlaubs_modus'] == 0 && ($USER[$resource[$Element]] < $pricelist[$Element]['max'] && IsTechnologieAccessible($USER, $PLANET, $Element) && in_array($Element, $reslist['tech'])) || $TheCommand == "cancel" || $TheCommand == "remove")
		{redirectTo('game.php?page=buildings&mode=research');}
		...
		foreach($reslist['tech'] as $ID => $Element)
		{
			...
			if(isset($pricelist[$Element]['max']) && $USER[$resource[$Element]] >= $pricelist[$Element]['max']) {
				$TechnoLink  =	'<img data-role="PGtooltip" data-direction="top" data-content="<table><td>'.$LNG['bd_maxlevel'].'</td></table>" src="styles/theme/' .$skin_raza .'/imagenes/navegacion/construir_red.gif" />';
			} elseif($CONF['queue_research_commander'] > 1) {
				$LevelToDo 	= 1 + $USER[$resource[$Element]];
				$TechnoLink = $CanBeDone && $bContinue && $CONF['queue_research_commander'] != count($TechQueue) ? '<a href="game.php?page=buildings&amp;mode=research&amp;cmd=insert&amp;tech='.$Element.'"><img data-role="PGtooltip" data-direction="top" data-content="<table><td>'.(($USER['b_tech_id'] != 0) ? $LNG['bd_add_to_list'] : $LNG['bd_research'].(($LevelToDo == 1) ? '' : ' '.$LNG['bd_lvl'].' '.$LevelToDo)).'</td></table>" src="styles/theme/' .$skin_raza .'/imagenes/navegacion/construir.gif" /></a>' : '<img data-role="PGtooltip" data-direction="top" data-content="<table><td>'.$LNG['bd_research'].(($LevelToDo == 1) ? '' : ' '.$LNG['bd_lvl'].' '.$LevelToDo).'</td></table>" src="styles/theme/' .$skin_raza .'/imagenes/navegacion/construir_red.gif" />';
				...
				if ($USER['b_tech_id'] == 0) {
					...
					$TechnoLink = $CanBeDone && $bContinue ? '<a href="game.php?page=buildings&amp;mode=research&amp;cmd=insert&amp;tech='.$Element.'"><img data-role="PGtooltip" data-direction="top" data-content="<table><td>'.$LNG['bd_research'].(($LevelToDo == 1) ? '' : ' '.$LNG['bd_lvl'].' '.$LevelToDo).'</td></table>" src="styles/theme/' .$skin_raza .'/imagenes/navegacion/construir.gif" /></a>' : '<img data-role="PGtooltip" data-direction="top" data-content="<table><td>'.$LNG['bd_research'].(($LevelToDo == 1) ? '' : ' '.$LNG['bd_lvl'].' '.$LevelToDo).'</td></table>" src="styles/theme/' .$skin_raza .'/imagenes/navegacion/construir_red.gif" />';
			...
			if (count($TechQueue) >= $CONF['queue_research_init'] and $USER['commander'] <= 0) {
			...
			$ResearchList[] = array(
				...
				'link'  	=> $TechnoLink,
				'irv' => $irv,
				'oldlink'  	=> $CONF['queue_research_commander'] == 1,
				'queue'  	=> $TechQueue,
			);
		}

		$Build 		= "<center>" .$LNG['ov_free_inves'] ."</center>";
		...
		$template->assign_vars(array(
			...
			'ov_lab_edif'		=> $LNG['ov_planets_inv'],
			'build'				=>$Build,
            'rese'                  => true
		));
